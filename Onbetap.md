...
- **Q** (panel label; "Resonance" internally) — 0–100%. Self-oscillation onset sits around 3/4 of the knob's travel, earlier at high cutoff (the real op-amps lose damping as their internal poles rise with cutoff, so onset *moves* — a behavior no open-source emulation had modeled before this one). CV input scaled ±10 V by its attenuverter.
- **Drive** — 0–100%, mapped to roughly −12…+24 dB of gain into the nonlinear core. Driving harder pushes the core into saturation and, characteristically, **suppresses resonance** — a loud signal rings noticeably less than a quiet one at the same Q setting, matching the hardware. CV input scaled by its attenuverter.
...
- **Resonance limiting** — **Hard** (default, factory/Erica rails) or **Soft** (diode-clamp mod). Hard self-oscillation rides the rails and comes out square-ish; Soft self-oscillation is rounder and sits a bit higher in pitch — measured at 308 Hz (Hard) vs. 362 Hz (Soft) under identical conditions.
...
- **Tuning** — four sliders exposing the calibration constants directly, for by-ear adjustment without a rebuild:
  - **Drive span** (24–48 dB, default 36 dB) — the dB range the Drive knob covers.
