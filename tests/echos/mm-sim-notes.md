# Échos — MetaModule build + headless simulator verification

Task 12 of the beads-delay plan. Covers: `.mmplugin` build, headless-simulator
DSP verification (echo delay-time math vs. measured output), and CPU
measurement.

Scripts and patch fixtures live in `tests/echos/mm_sim/` (`gen_burst.py`,
`read_mm_wav.py`, `gen_load_patch.py`, `echos_default.yml`,
`echos_load_16.yml`). WAV outputs are not committed (regenerate with the
commands below); they're multi-MB and fully reproducible.

## 1. `.mmplugin` build

Added to `metamodule/CMakeLists.txt`:
- `${SRC}/src/beadsdelay/Echos.cpp` under "VCV-adapter modules".
- The four beadsdelay DSP `.cpp` files individually (CMake here doesn't
  glob): `echos_processor.cpp`, `time/base_time.cpp`, `engine/echo_engine.cpp`,
  `pitch/rotary_shifter.cpp` (`mod/` and `env/` are header-only, per the task
  brief, so nothing to add there).
- Include dirs: `${SRC}/src/beadsdelay`, `${SRC}/src/beadsdelay/dsp/include`.
...
directory` from both `echos_processor.h` and `engine/echo_engine.h`. Échos
...
(`vcv/Makefile:32-33`: `-I../src/beadsdelay/dsp/include -I../src/beadsdelay/dsp/src -I../src/particules/dsp/src`,
commented "mirrors tests/beadsdelay_dsp/CMakeLists.txt's four include dirs").
Mirrored the same two extra include dirs
(`${SRC}/src/particules/dsp/src`, `${SRC}/src/beadsdelay/dsp/src`) into
...
Result: clean build (no warnings from Échos code — the only warnings in the
...
was already passing before this task; beadsdelay's own file names/symbols
...
cmake -B build-headless -GNinja -DHEADLESS=ON \
  -Dext_builtin_brand_paths="/Users/gabrielroth/Dev/RobotBoy/.worktrees/worktree-beads-delay/metamodule" \
...
`Echos.cpp` (mirroring `Particules.cpp`'s `memalign()` guard) took the
...
Slug is `RobotBoy:Echos` (plugin.json's top-level slug + Échos's `createModel`
...
no clock connected to `SEED_INPUT` in this patch):
...
cd tests/echos/mm_sim
python3 gen_burst.py audio_in.wav
/Users/gabrielroth/Dev/metamodule/simulator/build-headless/simulator \
  -p echos_default.yml -n 192000 -i audio_in.wav -o audio_out.wav
python3 read_mm_wav.py audio_out.wav
```

## 5. CPU measurement

Single cabled `Echos` instance (`echos_default.yml`), silence input (signal
...
run. This suggests Échos's cost is not (or not fully) incurred by an
...
`gen_load_patch.py`'s "no cabling needed" assumption, rather than a Échos bug.

## Summary

| Check | Result |
|---|---|
| `.mmplugin` build | Clean; needed 2 extra include dirs beyond the brief (`particules/dsp/src`, `beadsdelay/dsp/src`) to find `RecordingBuffer` |
