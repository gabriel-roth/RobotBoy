#!/usr/bin/env python3
"""Generate an N-instance Echos-only patch (no cabling needed -- the patch
...
out_path = sys.argv[2] if len(sys.argv) > 2 else f"echos_load_{N}.yml"

lines = []
lines.append("PatchData:")
lines.append(f"  patch_name: echos_load_{N}")
lines.append(f"  description: Echos CPU load test, {N} instances")
lines.append("  module_slugs:")
lines.append("    0: '4msCompany:HubMedium'")
for i in range(1, N + 1):
    lines.append(f"    {i}: 'RobotBoy:Echos'")
