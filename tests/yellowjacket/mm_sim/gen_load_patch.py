#!/usr/bin/env python3
"""Generate an N-instance Yellowjacket-only patch (no cabling needed -- the
...
out_path = sys.argv[3] if len(sys.argv) > 3 else f"yj_load_{N}.yml"

lines = []
lines.append("PatchData:")
lines.append(f"  patch_name: yj_load_{N}")
lines.append(f"  description: Yellowjacket CPU load test, {N} instances" + (f", osMenu={os_menu}" if os_menu is not None else ""))
lines.append("  module_slugs:")
lines.append("    0: '4msCompany:HubMedium'")
for i in range(1, N + 1):
    lines.append(f"    {i}: 'RobotBoy:Yellowjacket'")
