# Yellowjacket — MetaModule build + headless simulator verification
...
Scripts and patch fixtures used below live in `tests/yellowjacket/mm_sim/`
(`gen_saw.py`, `read_mm_wav.py`, `gen_load_patch.py`, `yj_*.yml`). WAV outputs
...
correctly in the same target. Yellowjacket's own `#ifndef METAMODULE` slider
...
Result: clean build, no Yellowjacket-related errors (the module's new source
has no `try/catch/throw`/iostream patterns needing MM-incompatibility fixes).
`RobotBoy.mmplugin` produced (703 KB); `tar -tf` shows
`RobotBoy/Yellowjacket.png` and `RobotBoy/Yellowjacket-gold.png` bundled
correctly (metamodule/assets and metamodule/plugin-mm.json already had
Yellowjacket entries from a prior task — no scaffolding needed).
...
**Simulator-repo gap found (not a Yellowjacket bug):** `simulator/CMakeLists.txt`
...
issue outside Yellowjacket's scope; flagging here rather than fixing it there.
...
- `HostFileIO::normalize_path` mishandles absolute paths (an input starting
  ...
  `../../RobotBoy/.worktrees/worktree-wasp/tests/yellowjacket/mm_sim/foo.yml`),
...
- **Module ID 0 is always the Hub/panel module** (`patch_player.hh`:
  ...
  starts at `i = 1`). A `module_slugs: {0: 'RobotBoy:Yellowjacket'}` patch
...
- The registered brand string is the **plugin's `slug`** (`plugin.json`'s
  ...
  patch slug string: `RobotBoy:Yellowjacket`.
...
the patch runs Yellowjacket's true-stereo path, not the R-normalled-to-L
shortcut). Output: LP L/R captured via `mapped_outs`.

**Default patch** (`yj_default.yml`: Tame, Res=0, Freq≈750 Hz default, no
...
**Screaming + Resonance 0.95 patch** (`yj_res_screaming.yml`:
...
cd tests/yellowjacket/mm_sim
python3 gen_saw.py audio_in.wav
cd /Users/gabrielroth/Dev/metamodule/simulator
D=../../RobotBoy/.worktrees/worktree-wasp/tests/yellowjacket/mm_sim
build-headless/simulator -p $D/yj_default.yml -n 144000 --in $D/audio_in.wav --out $D/audio_out_default.wav
build-headless/simulator -p $D/yj_res_screaming.yml -n 144000 --in $D/audio_in.wav --out $D/audio_out_res_screaming.wav
```
...
|---|---|---|
| 1x (`yj_os1.yml`) | ~2090 ms | ~0.70% |
| 2x (`yj_os2.yml`) | ~7067 ms | ~2.36% |
| 4x (`yj_default.yml`, pre-cap auto) | ~15433 ms | ~5.14% |
| 4x, mono (R normalled to L, `yj_os4_mono.yml`) | ~7836 ms | ~2.61% |

Each figure is the average of 2 repeated runs (all within ~1% of each other —
stable). A 16-instance load-test patch (`yj_load16_os{1,2,4}.yml`,
...
(`src/Yellowjacket.cpp`). 1x/2x/4x remain manually selectable from the
...
MM-side discrete-choice fallbacks in `src/Yellowjacket.cpp`'s
...
| Check | Result |
|---|---|
| `.mmplugin` build | Clean, Yellowjacket compiles with no MM-incompatible patterns |
