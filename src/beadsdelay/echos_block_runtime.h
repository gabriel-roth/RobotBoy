...
#include "beadsdelay_dsp/types.h"

#include <algorithm>
#include <array>
#include <cstddef>

// Block accumulator for the Echos VCV adapter. Data-path only (mirrors the
...
// ReadOutputSample. Grain LED and trigger-pulse machinery are dropped (Echos
// has no grains); clock/tap edge tracking is added instead.
template <size_t BlockSize>
class EchosBlockRuntime {
public:
    static_assert(BlockSize >= 1, "Echos block runtime requires BlockSize >= 1");

    bool PushInputSample(beadsdelay_dsp::StereoFrame in) {
    ...
    beadsdelay_dsp::StereoFrame* InputBuffer() { return input_buf_.data(); }
    const beadsdelay_dsp::StereoFrame* InputBuffer() const { return input_buf_.data(); }

    void CommitProcessedBlock(const beadsdelay_dsp::StereoFrame* processed, size_t count) {
    ...
    beadsdelay_dsp::StereoFrame ReadOutputSample() {
        beadsdelay_dsp::StereoFrame out = output_buf_[output_index_];
        ++output_index_;
    ...
    // reaches it first) into EchosParameters::clock_tick_offset. Consuming
...
private:
    std::array<beadsdelay_dsp::StereoFrame, BlockSize> input_buf_ {};
    std::array<beadsdelay_dsp::StereoFrame, BlockSize> output_buf_ {};
