...
struct EchosWidget : ModuleWidget {
	...
	EchosWidget(Echos* module) {
		...
		// Placeholder layout on a 12 HP grid — Task 11 finalizes the real
		// panel and widget positions (res/Echos.svg here is just a copy of
		// Ondes.svg, narrower than 12 HP, so some widgets sit past its right
		// edge for now; that's expected until Task 11 lands). 4 columns
		// x = 8.6/23.6/38.6/53.6 mm; 3 knob rows (12 params, one per cell) at
		// y = 42/62/82 mm; jacks (12 in/out) fill 3 more rows at the given
		// cadence, y = 100/113/126 mm (126 extends the brief's two jack rows
		// by the same 13 mm step to fit all I/O without overlapping slots).
		static constexpr float kCol[4] = {8.6f, 23.6f, 38.6f, 53.6f};

		addParam(createLightParamCentered<VCVLightLatch<MediumSimpleLight<WhiteLight>>>(mm2px(Vec(kCol[0], 42.f)), module, Echos::FREEZE_PARAM, Echos::FREEZE_BUTTON_LIGHT));
#ifdef METAMODULE
		addParam(createParamCentered<QualityMmSwitch>(mm2px(Vec(kCol[1], 42.f)), module, Echos::QUALITY_PARAM));
#else
		addParam(createLightParamCentered<VCVLightBezel<RedGreenBlueLight>>(mm2px(Vec(kCol[1], 42.f)), module, Echos::QUALITY_PARAM, Echos::QUALITY_R_LIGHT));
#endif
		addParam(createLightParamCentered<VCVLightBezel<WhiteLight>>(mm2px(Vec(kCol[2], 42.f)), module, Echos::SEED_PARAM, Echos::SEED_LIGHT));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[3], 42.f)), module, Echos::DENSITY_PARAM));

		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[0], 62.f)), module, Echos::TIME_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[1], 62.f)), module, Echos::PITCH_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[2], 62.f)), module, Echos::SHAPE_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[3], 62.f)), module, Echos::FEEDBACK_PARAM));

		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(kCol[0], 82.f)), module, Echos::DRY_WET_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(kCol[1], 82.f)), module, Echos::TIME_AR_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(kCol[2], 82.f)), module, Echos::PITCH_AR_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(kCol[3], 82.f)), module, Echos::SHAPE_AR_PARAM));

		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[0], 100.f)), module, Echos::IN_L_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[1], 100.f)), module, Echos::IN_R_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[2], 100.f)), module, Echos::FREEZE_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[3], 100.f)), module, Echos::SEED_INPUT));

		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[0], 113.f)), module, Echos::DENSITY_CV_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[1], 113.f)), module, Echos::TIME_CV_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[2], 113.f)), module, Echos::PITCH_CV_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[3], 113.f)), module, Echos::SHAPE_CV_INPUT));

		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[0], 126.f)), module, Echos::FEEDBACK_CV_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(kCol[1], 126.f)), module, Echos::DRY_WET_CV_INPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(kCol[2], 126.f)), module, Echos::OUT_L_OUTPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(kCol[3], 126.f)), module, Echos::OUT_R_OUTPUT));
	}
};
