...
**Particules** is a granular texture processor for [VCV Rack](https://vcvrack.com) and the [4ms MetaModule](https://4mscompany.com/metamodule). It continuously records your incoming audio into a buffer, then plays back a swarm of short overlapping fragments — *grains* — each with its own position, pitch, length, and envelope. Results range from lush self-generating clouds to tightly clocked rhythmic slicing. It's inspired by Mutable Instruments Beads.
...
2. **Turn Density** away from center. At 12 o'clock no grains are created. Turn it clockwise for a cloud with randomly distributed grains, or counter-clockwise for a steady pulse of grains. The further from center, the more grains.
...
*Size:* Grain duration and direction. Around 11 o'clock, grains are very short (\~30 ms). Turn clockwise for longer grains, up to several seconds; counter-clockwise for *reversed* grains, likewise up to several seconds.
...
Below **Time, Size, Shape,** and **Pitch** sit the *attenurandomizers.* Each does one of two things, depending on whether a cable is in that parameter's CV input:
...
- **Seed CV mode** — how the module responds to CV at the Seed input. In **Triggers** mode (default), each incoming trigger spawns a grain — or several, or the probability of one — depending on Density; with nothing patched into Seed, Particules free-runs on Density alone. In **Gates** mode, grains fire only while the gate is high, with **Density** setting their repetition rate during the gate; at 12 o'clock exactly one grain fires per gate. **Gates mode needs a cable in Seed:** unpatched, the gate never goes high, so no grains are generated at all.
