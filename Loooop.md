...
4. Turn up **Dry/Wet** toward *Wet* if you only want to hear the loop, and take your sound from **Mix L / Mix R** (bottom right).

That's the whole loop. Everything else is about shaping what those four heads do with it.
...
All four are summed into the **Mix** output — each positioned in the stereo field by its own **Pan** knob — and each head also has its **own pair of outputs** if you want to process them separately.
...
| **Level** | This head's volume in the mix. (Defaults are set so all four heads at once add up to a normal level.) |
...
| **Pan** | Where this head sits in the stereo **Mix** — left, center (default), or right. It only affects the Mix; the head's own **Out L / R** jacks are left untouched. |
...
- **Record** — Starts and stops recording. The first press records a fresh loop. Once a loop exists, pressing Record again **overdubs** (layers new audio on top of what's there), if overdubbing is enabled (see the right-click menu). The red light shows when it's recording.
...
Loooop is built for modular control. Nearly every knob has a **CV input** right below or beside it, and a full ±10 V sweep on a CV input covers that knob's entire range (the CV adds to wherever you've set the knob).

**Global jacks (bottom row):**

- **In L / In R** — audio in. Patch one for mono; it feeds both channels.
- **Record trigger** / **Clear trigger** — a trigger or gate here does the same as clicking the Record / Clear buttons, so you can drive them from a clock or sequencer.
...
- **Out L / Out R** — that head's own isolated stereo output.
...
You can find a few extra options by right-clicking the panel in VCV Rack or scrolling down to Options in MetaModule:
...
*Loooop and Löp are released under the MIT license.*
