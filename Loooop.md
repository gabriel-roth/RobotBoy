...
- **Record jack** — Trigger (the default) or Gate. In **Trigger** mode, the **Record trigger** input works exactly like a second press of the Record button: a rising edge on the jack (or the button) starts recording, and another rising edge closes the pass, following **When recording ends** above. In **Gate** mode, the jack becomes a punch switch instead: the pass opens on the jack's rising edge and closes on the falling edge — the same "close" behavior a second trigger gives today, just tied to the gate's two edges instead of two triggers. If the jack goes high again while a pass is still open (for example, after **Keeps overdubbing** rolled the previous pass straight into overdubbing), that's a punch: the open pass closes and a fresh one starts immediately, in the same instant. With **Keeps overdubbing**, the gate alone can never settle into plain playback on its own — a falling edge always re-enters overdub, a rising edge always punches — so the panel **Record** button is the escape hatch back to a stopped, playing loop; it always works as a simple press-to-toggle, in either Record jack mode.
