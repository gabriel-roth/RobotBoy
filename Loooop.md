...
**Loooop** is a stereo, RAM-based looper for [VCV Rack](https://vcvrack.com) and the [4ms MetaModule](https://4mscompany.com/metamodule). You feed it audio, capture a loop, and then play that loop back with **four independent playheads,** each with its own speed, position, window size, level, jitter, and stereo pan. One recorded phrase becomes four loops at four different speeds and lengths, layered into a single stereo mix. It's inspired by [Cutlasses Gloop](https://www.cutlasses.co.uk/product/gloop/).
...
The strip across the top shows the recorded audio as a waveform. While recording, the captured audio is stretched across the full width, compressing as it grows. Once you have a loop, you'll see up to four colored **playhead markers** sweeping across it, along with a shaded band showing each head's *window* (the slice of the loop that head plays). It's the easiest way to see what Position, Size, and Speed are doing.
...
All four are summed into the **Mix** output, and each head also has its own outputs for independent processing.
...
- **Record** — Starts and stops recording. The first press records a fresh loop; once a loop exists, pressing Record again **overdubs**, with the **Overdub** button deciding what happens to the audio already there. The red light shows when Loooop is recording.
- **Overdub** — A five-way button that selects one of the overdub modes:
    - **Layer** (blue, the default) — sound-on-sound: each overdub pass turns the existing material down slightly (about 1 dB per pass), so older layers gradually sink underneath newer ones.
    - **Decay** (orange) — like Layer, but older layers also lose a little high end on every pass, for tape-style degradation.
    - **Add** (green) — new audio sums with what's there at full level, forever, so it eventually clips and distorts.
...
- **Jump** — a control voltage that snaps the playhead to a spot within its window (0V = start of window, 10V = end). Great for stutter and glitch driven by an LFO or sequencer. (Jump follows the *voltage*, not trigger edges: a bare trigger just drops the head back at the window start when the pulse falls, so hold the voltage between steps and amplify to the full 0–10V range.)
...
Checking a playhead under **One-shot on trigger** in the context menu changes that: the head stays silent until triggered, then plays **through its window exactly once and stops**, ending with a short fade so it doesn't click. While it waits for a trigger, its lane on the display **dims** — so if a playhead ever seems dead, it's probably in one-shot mode.

---

## Context menu

You can find these options in the context menu in VCV Rack or scrolling down to Options in MetaModule:

- **When recording ends** — sets what happens when a recording pass closes. **Plays back** (the default) freezes the loop and starts playback. **Keeps overdubbing** freezes the loop length at that moment and rolls straight on into an overdub pass, so a single clock, trigger stream, or gate can capture a loop and keep layering it without a second command. (In **Lock** overdub mode, overdubbing is off, so it always just stops.)
- **Record jack** — Trigger (the default) or Gate. In **Trigger** mode, the **Record trigger** input works exactly like a second press of the Record button: a rising edge on the jack, or a press of the button, starts or closes a recording pass, following **When recording ends** above; jack and button fire independently, so either still works while the other is held. In **Gate** mode the jack drives recording directly: a pass opens on the rising edge and closes on the falling edge, with a rise during an open pass ignored. Combined with **Keeps overdubbing**, that gives a gate stream a three-step lifecycle — the first pulse records the loop and rolls into overdubbing when it falls, the second pulse's fall stops the overdub and leaves the loop playing, and every pulse after that is a plain overdub punch-in/punch-out. The Record button is always a simple press-to-toggle in either mode. (Gate thresholds differ by host: VCV registers high around 2 V and low around 0.1 V; MetaModule registers high above 1 V, with no separate falling threshold.)
...
- **One-shot on trigger** — check a playhead to put it in one-shot mode (see above).
...
- **Crossfade** — When on (the default), Loooop applies a tiny (\~5 ms) fade at each loop's wrap-around point to hide clicks, and gives **one-shot** passes a matching fade-out at the end instead of a hard stop. Turn it off for the raw, seam-exact repeat (useful for rhythmic clicks or very short grains).
