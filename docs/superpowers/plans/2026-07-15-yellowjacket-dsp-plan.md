...
// kHalfbandTaps: use the 47-tap primary array from fitted_constants.md.
...
- [ ] **Step 2: Wire `src/Yellowjacket.cpp`.** Replace the passthrough `process()` with the MF-20 pattern:
  ...
  - Menu additions in `appendContextMenu` after Character: Accuracy (Standard/High), Oversampling (Auto/1x/2x/4x — changing it re-runs the `onSampleRateChange` logic; call a shared `updateOversampling()`), Input trim (`createMenuItem` slider — use `rack::ui::Slider` with a small `Quantity` subclass, range ±12 dB, default 0, label "Input trim"); Inverter bandwidth (`rack::ui::Slider`, log-scaled 60–300 kHz, default 80 kHz, label "Inverter bandwidth" — feeds fPole in the kC2eff formula, i.e. how eagerly the filter self-oscillates; floor is 60 kHz — Task 1's corrected sweep found Tame free-runs below a threshold between 55 and 60 kHz); Self-osc pitch (two `createMenuItem` entries: "Hardware (drifts flat)" default / "Corrected (tracks knob)" — when corrected and Screaming, scale fcInt by `1 + (1/0.72 − 1)·s` where `s = clamp((rho − 0.85)/0.15, 0, 1)` smoothed, so the rail-bounded limit cycle (~0.72·knob per Task 1 golden data) lands on the knob at rho=1 while low-rho small-signal placement is untouched). Persist `_highAcc`, `_osMenu`, `_inputTrimDb`, `_fPole`, `_oscPitchCorrected` in dataTo/FromJson alongside existing fields.
